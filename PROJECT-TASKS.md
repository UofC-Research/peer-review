...
| 2     | Complete | Data acquisition foundation              | Implemented metadata extract/load for bioRxiv/medRxiv, deterministic acquisition requests, initial preprint version selection, published DOI carry-forward, and matched full-text download workflows. | Extend published-article full-text resolution beyond DOI landing pages to publisher-specific PDF, XML, and HTML sources.  |
...
| `peer_elt.acquire.corpus`        | Covered by corpus acquisition and acquired-artifact scoring tests | Complete for current Study 1 acquisition workflow      | Extend publisher-specific full-text resolution                                              |
| `peer_elt.acquire.full_text`     | Covered by full-text acquisition tests                            | Complete for configured PDF/XML/HTML fallback requests | Add publisher-specific published article PDF/XML/HTML discovery                             |
