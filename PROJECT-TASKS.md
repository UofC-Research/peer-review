...
- `conda run -n peer_review_env python -m pytest` -> 87 passed, 3 live tests skipped, with one cache-permission warning
  for `.pytest_cache`
- `conda run -n peer_review_env python -m pytest --cov=peer_elt --cov-report=term-missing -q` -> 87 passed, 3 skipped,
  88% total
...
| `peer_elt.acquire.tdm`           | Covered by TDM acquisition tests                                  | Complete for injectable bioRxiv/medRxiv TDM archive sync, lightweight AWS requester-pays probes, published-link metadata download, and XML-first published full-text retrieval automation | Wire full archive sync into production CLI/jobs only if bulk TDM retrieval is selected      |
...
- Added config-driven opt-in live integration tests for preprint APIs, published full-text retrieval, and matched-pair
  acquisition
- Added TDD-covered optional TDM acquisition automation for bioRxiv/medRxiv requester-pays preprint archive sync,
  bioRxiv API published-link metadata download, and XML-first published article full-text retrieval
- Added disabled-by-default AWS/TDM acquisition settings to local and production configs plus opt-in live AWS
  requester-pays bucket probes
