...
- Python: `conda run -n peer_review_env python -m pytest` -> 87 passed,
  3 live tests skipped when no live config is supplied
- Python coverage: `conda run -n peer_review_env python -m pytest --cov=peer_elt --cov-report=term-missing -q` -> 87
...
intended for bulk preprint full-text retrieval plus automated linked published
metadata and published full-text retrieval. When TDM is selected, the preferred
processing order for matched preprint and published article files is XML, then
PDF, then HTML, because XML supports more efficient structured comparison than
PDF and HTML should remain an automated fallback rather than a manual
